create or replace function public.outbreak_stats(_days integer default 180)
returns jsonb
language plpgsql
stable
security definer
...
declare
  ...
  _res jsonb;
begin
  if not public.has_role(auth.uid(), 'ANALYST') then
    raise exception 'Analyst role required';
  end if;
  ...
  daily as (
    select
      ...
      count(*) as total,
      count(*) filter (where primary_icd_10 like 'U07%') as covid,
  ...
  postal as (
    select
      ...
      count(*) filter (where encounter_date >= now() - interval '14 days'
                         and primary_icd_10 like 'U07%') as recent_covid
...
grant execute on function public.outbreak_stats(integer) to authenticated;